...
public class FactoryExample {
	...
	public void getResult(Factory mainFactory, char mainFactorySkill, IWorkingTogether partner) {
		...
		if(mainFactory instanceof CarFactory){
			cf = (CarFactory)mainFactory;
			System.out.println("2. 파트너 공장 [" + cf.name(partner) + "] 협력 시 추가 생산량 = " + cf.workTogether(partner));
		}
		else if(mainFactory instanceof TVFactory){
			tf = (TVFactory)mainFactory;
			System.out.println("2. 파트너 공장 [" + tf.name(partner) + "] 협력 시 추가 생산량 = " + tf.workTogether(partner));
		}
		System.out.println("--------------------------------------------------------------");
	}
